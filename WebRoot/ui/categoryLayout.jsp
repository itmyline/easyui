...
<html>
		...
		$("#dom_var_pagination").pagination( {
			 onSelectPage: function(pageNumber, pageSize){ 
	            //$('#tt').datagrid('load', "getUserAction.action?currentPage="+pageNumber+"&pageSize="+pageSize+"");
			 	$("#tt").datagrid( {
					url : 'getCategoryAction.action?result.currentPage='+pageNumber+'&result.pageSize='+pageSize+'',
					closable : true,
					checkOnSelect : true,
					striped : true,
					rownumbers : true,
	...
	$("#a_var_search").bind('click', function() {
		...
		var state = $("#state").val();
		$("#tt").datagrid( {
			url : 'getCategoryAction.action?entity.name='+name+'&entity.state='+state,
			closable : true,
			checkOnSelect : true,
			striped : true,
			rownumbers : true,
	...
	function shortMenuOpen_grid(rowIndex, menuItem){
		if (menuItem.name == 'add') {
			...
			$("#dom_var_window").window('refresh', "categoryShow.action?pageType=1");
		}
		if (menuItem.name == 'del') {
			...
			if(selectedArray.length>0){
				...
				$.messager.confirm('提示',str,function(flag){
					if(flag){
						$.post('categoryDelAction.action?ids='+ids,{},function(f){
							if(f.state=='1'){
		...
		if (menuItem.name == 'edit') {
			...
			if(selectedArray.length==1){
				...
				$("#dom_var_window").window('open'); 
				$("#dom_var_window").window('refresh', "categoryShow.action?ids='"+selectedArray[0].id+"'&pageType=2");
			}else{
		...
		if (menuItem.name == 'view') {
			...
			if(selectedArray.length==1){
				...
				$("#dom_var_window").window('open'); 
				$("#dom_var_window").window('refresh', "categoryShow.action?ids='"+selectedArray[0].id+"'&pageType=0");
			}else{
				$.messager.alert('提示', '请选择一条记录!', 'info');  //提示信息
			}
		}
